...
## מה תוקן (18.08) - חסימה אמיתית ברמת מערכת ההפעלה

**הבעיה שהתגלתה:** חסימת תהליכים לפי שם (`taskkill` על `vlc.exe` וכו') היא הגנה חלשה — כל מי שמשנה שם קובץ `.exe`, או משתמש בנגן שלא ברשימה, עוקף אותה בקלות. זו לא הייתה "חסימה מוחלטת" כפי שהתבקש.

**הפתרון:** נוסף מודול חדש (`agent/src/fileLock.js`) שחוסם את **קבצי הווידאו עצמם** ברמת ה-NTFS (הרשאות קבצים של Windows), באמצעות `icacls`. זה אומר ש**אף תוכנה, לא משנה שמה**, יכולה לפתוח את הקובץ — כי Windows עצמו מסרב, לפני שכל נגן בכלל מקבל הזדמנות לרוץ. ה-agent סורק את כל הכוננים (קבועים ונשלפים) בהתקנה ובאופן תקופתי, ונועל כל קובץ עם סיומת וידאו (`.mp4`, `.mkv`, `.avi` ועוד — הרשימה המלאה ב-`config.js`).

חסימת התהליכים הישנה (`blocker.js`) **נשארה כהגנה משנית** בנוסף לזה, ולא הוחלפה.

### מגבלות אמיתיות של הגישה הזו (חשוב לדעת)

1. **לא חוסם סטרימינג מהדפדפן** (Netflix, YouTube וכו') — אין קובץ מקומי לנעול, כי התוכן זורם ישירות מהרשת. לחסום את זה דורש חסימת דפדפנים שלמים או סינון ברמת הרשת/DNS — שינוי היקף משמעותי מעבר ל"חסימה מקומית במחשב" המקורית. אם זה נדרש, זה אפשרי כתוספת נפרדת (למשל חסימת `chrome.exe`/`msedge.exe` דרך "ערוך רשימת חסימה" בדשבורד, במחיר של חסימת כל הגלישה, לא רק וידאו).
2. **לא חוסם משתמש עם הרשאות Administrator באותו מחשב** — הוא יכול תיאורטית להחזיר לעצמו הרשאות (Take Ownership) ולבטל את הנעילה. זו מגבלה מובנית של מודל ההרשאות של Windows עצמו — אין שום תוכנה (כולל אנטי-וירוס מסחרי) שיכולה למנוע ממנהל מערכת לשנות הרשאות במחשב שהוא אדמין בו. ההגנה הזו מיועדת למשתמש רגיל (לא-אדמין) על המחשב.
3. **כוננים ברשת (network shares) לא נסרקים** — בכוונה, כדי לא לשנות הרשאות על משאב משותף שאנשים אחרים תלויים בו.
4. **פתיחה זמנית / הסרה משחררת את כל הקבצים הנעולים בחזרה** — זה מכוון: אנחנו לא רוצים להשאיר קבצים של מישהו נעולים לצמיתות אחרי שהתוכנה הוסרה.

---

## מה תוקן (17.08, עדכון שני) - הסרה מרחוק, לוגים, ורשימת חסימה

**באג בהסרה מרחוק:** סקריפט ה-uninstall השתמש בפקודת `timeout` כדי להמתין רגע לפני הסרת השירות. הפקודה הזו דורשת קונסולה אמיתית כדי לפעול, ונכשלת מיידית כשהיא מופעלת "מנותקת" (detached) כמו במקרה שלנו - כך שהסקריפט פשוט נעצר לפני שהגיע בכלל לפקודות ההסרה בפועל. הוחלף ב-`ping` (טריק סטנדרטי להמתנה בלי תלות בקונסולה).

**רשימת חסימה מורחבת + ניתנת לעריכה:** נוספה תמיכה באפליקציית הווידאו המובנית של Windows (Movies & TV). חשוב מכך - נוסף כפתור **"ערוך רשימת חסימה"** בדשבורד לכל מחשב בנפרד, כי אין דרך אחת שמכסה את כל התוכנות האפשריות (וגם כי חסימת דפדפנים שלמים, למרות שהיא הדרך היחידה לתפוס סטרימינג מהדפדפן, היא החלטה גדולה מדי בשביל להיות ברירת מחדל אוטומטית).

**מערכת לוגים חדשה:** ה-agent שומר כעת לוג מקומי (פעולות חסימה, שגיאות, אירועי מערכת). בדשבורד יש כפתור **"בקש לוגים"** (שולח בקשה, המחשב מעלה בפעם הבאה שהוא מתחבר) וכפתור **"הורד לוגים"** (מוריד קובץ טקסט עם התוכן שהועלה לאחרונה).
...
**היקף ה"הסרה מרחוק" מהדשבורד:** פקודת "הסר תוכנה" מהדשבורד עוצרת ומסירה את השירות (כך שהחסימה נפסקת) ומוחקת את זהות המכשיר המקומית — אבל לא מוחקת את קבצי ההתקנה עצמם. להסרה מלאה של הקבצים, עדיין אפשר דרך "הוספה/הסרה של תוכניות" בווינדוס.
